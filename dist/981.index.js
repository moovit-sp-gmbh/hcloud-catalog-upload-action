...
var credentialProviderLogin = __webpack_require__(7751);

const resolveCredentialSource = (credentialSource, profileName, logger) => {
    const sourceProvidersMap = {
        EcsContainer: async (options) => {
            const { fromHttp } = await __webpack_require__.e(/* import() */ 277).then(__webpack_require__.bind(__webpack_require__, 8277));
            const { fromContainerMetadata } = await __webpack_require__.e(/* import() */ 385).then(__webpack_require__.t.bind(__webpack_require__, 8385, 19));
            logger?.debug("@aws-sdk/credential-provider-ini - credential_source is EcsContainer");
            return async () => propertyProvider.chain(fromHttp(options ?? {}), fromContainerMetadata(options))().then(setNamedProvider);
        },
        Ec2InstanceMetadata: async (options) => {
            logger?.debug("@aws-sdk/credential-provider-ini - credential_source is Ec2InstanceMetadata");
            const { fromInstanceMetadata } = await __webpack_require__.e(/* import() */ 385).then(__webpack_require__.t.bind(__webpack_require__, 8385, 19));
            return async () => fromInstanceMetadata(options)().then(setNamedProvider);
        },
        Environment: async (options) => {
            logger?.debug("@aws-sdk/credential-provider-ini - credential_source is Environment");
            const { fromEnv } = await Promise.resolve(/* import() */).then(__webpack_require__.t.bind(__webpack_require__, 3659, 19));
...
const resolveAssumeRoleCredentials = async (profileName, profiles, options, callerClientConfig, visitedProfiles = {}, resolveProfileData) => {
    ...
    if (!options.roleAssumer) {
        const { getDefaultRoleAssumer } = await __webpack_require__.e(/* import() */ 886).then(__webpack_require__.t.bind(__webpack_require__, 2886, 23));
...
const resolveProcessCredentials = async (options, profile) => __webpack_require__.e(/* import() */ 443).then(__webpack_require__.t.bind(__webpack_require__, 7443, 19)).then(({ fromProcess }) => fromProcess({
    ...options,
    profile,
...
const resolveSsoCredentials = async (profile, profileData, options = {}, callerClientConfig) => {
    const { fromSSO } = await __webpack_require__.e(/* import() */ 15).then(__webpack_require__.t.bind(__webpack_require__, 8015, 19));
    return fromSSO({
        profile,
...
const isWebIdentityProfile = (arg) => Boolean(arg) &&
    ...
    typeof arg.role_arn === "string" &&
    ["undefined", "string"].indexOf(typeof arg.role_session_name) > -1;
const resolveWebIdentityCredentials = async (profile, options, callerClientConfig) => Promise.all(/* import() */[__webpack_require__.e(886), __webpack_require__.e(855)]).then(__webpack_require__.t.bind(__webpack_require__, 2855, 23)).then(({ fromTokenFile }) => fromTokenFile({
...
var propertyProvider = __webpack_require__(4717);
var sharedIniFileLoader = __webpack_require__(263);
var protocolHttp = __webpack_require__(3643);
...
class LoginCredentialsFetcher {
    ...
    async refresh(token) {
        const { SigninClient, CreateOAuth2TokenCommand } = await __webpack_require__.e(/* import() */ 332).then(__webpack_require__.t.bind(__webpack_require__, 1332, 23));
